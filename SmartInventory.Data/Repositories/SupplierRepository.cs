...
namespace SmartInventory.Data.Repositories
{
    public class SupplierRepository(DbConnectionFactory dbConnectionFactory) : ISupplierRepository
    {
        ...
        public async Task<SupplierPagedResponseDto> GetAllAsync(int pageNumber, int pageSize)
        {
            var suppliers = new List<SupplierViewDto>();
            int totalCount = 0;
            ...
            while (await reader.ReadAsync())
            {
                suppliers.Add(new SupplierViewDto(reader.GetInt32(0), reader.GetString(1), reader.GetString(2), reader.GetString(3)));
            }
            ...
            return new SupplierPagedResponseDto(suppliers, totalCount, pageNumber, pageSize);
        }
